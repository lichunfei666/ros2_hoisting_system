...
# 启动HTTP服务
echo "[2/5] 启动HTTP服务..."
cd "${HTTP_DIR}" && python3 -m http.server 8080 > /dev/null 2>&1 &
HTTP_PID=$!
echo "      HTTP服务已启动，PID: ${HTTP_PID}"

# 启动rosbridge服务
echo "[3/5] 启动rosbridge服务..."
/opt/ros/jazzy/lib/rosbridge_server/rosbridge_websocket > /dev/null 2>&1 &
ROSBRIDGE_PID=$!
echo "      rosbridge服务已启动，PID: ${ROSBRIDGE_PID}"

# 启动无人机节点（dji_psdk_node）
echo "[4/5] 启动无人机节点..."
ros2 run dji_psdk_wrapper dji_psdk_node > /dev/null 2>&1 &
DRONE_PID=$!
echo "      无人机节点已启动，PID: ${DRONE_PID}"

# 启动地面站节点
echo "[5/5] 启动地面站节点..."
ros2 run ground_station ground_station_node > /dev/null 2>&1 &
GROUND_STATION_PID=$!
...
echo "查看服务日志：使用 ros2 logs --node <节点名称>"